/**
 * 独立 Node test 入口 —— 使用 Node 22 内置 --experimental-strip-types
 *
 * 用法：
 *   node --experimental-strip-types --experimental-specifier-resolution=node \
 *        --test src/services/narrative-consistency/__tests__/standalone.test.ts
 *
 * 注：只导入纯函数模块（validator/auto-fix/fact-extractor/context-builder），
 * 不经过 index.ts barrel 以避免加载依赖 Electron IPC 的 canon-store。
 */
